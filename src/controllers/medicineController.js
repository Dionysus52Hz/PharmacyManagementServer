...
const getMedicineById = async (req, res) => {
    ...
    try {
        ...
        const [rows] = await connection.query('SELECT * FROM Medicine WHERE medicine_id = ?', [medicine_id]);
        if (rows.length === 0) {
...
const deleteMedicine = async (req, res) => {
    ...
    try {
        ...
        const [result] = await connection.query('DELETE FROM Medicine WHERE medicine_id = ?', [medicine_id]);
