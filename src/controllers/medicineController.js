...
const getMedicineById = async (req, res) => {
  ...
  try {
    const [rows] = await pool.query('SELECT * FROM Medicine WHERE medicine_id = ?', [medicine_id]);
    if (rows.length === 0) {
...
const deleteMedicine = async (req, res) => {
  const { medicine_id } = req.params;
  try {
    const [result] = await pool.query('DELETE FROM Medicine WHERE medicine_id = ?', [medicine_id]);
    if (result.affectedRows === 0) {
      return res.status(404).send('Medicine not found');
    }
