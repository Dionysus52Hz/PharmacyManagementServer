...
const getReceivedNotes = async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM ReceivedNotes');
    res.json(rows);
  } catch (error) {
...
const getReceivedNoteById = async (req, res) => {
  ...
  try {
    const [rows] = await pool.query('SELECT * FROM ReceivedNotes WHERE received_note_id = ?', [received_note_id]);
    if (rows.length === 0) {
...
const deleteReceivedNote = async (req, res) => {
  const { received_note_id } = req.params;
  try {
    const [result] = await pool.query('DELETE FROM ReceivedNotes WHERE received_note_id = ?', [received_note_id]);
    if (result.affectedRows === 0) {
      return res.status(404).send('Received note not found');
    }
